...
function  sendScore() {
    layer.open({
        title: '您的分数为' + score + ',请输入您的名字'
        , content: "<input type='text' maxlength='8' id='name_input'/>"
        , btn: ['确定', '取消']
        ,shadeClose:false
        , yes: function (index) {
            $.ajax
            ({
                data: {"score": score, "name": $("#name_input").val()},
                type: 'post',
                url: "/Restaurant/score/add",
                dataType: 'json',
                success: function (data) {
                    ...
                    for(var k in data)
                    {
                        ...
                        var timetd = c$("td");
                        var scoretd = c$("td");
                        $(nametd).text(data[k].name);
                        $( scoretd).text(data[k].score);
                        $(timetd).text(data[k].time);
                        $(tr).append($(nametd));
                        $(tr).append($(scoretd));
                        $(tr).append($(timetd));
                        $(tr).appendTo($("#tbody"));
